import otfusion.parameters as parameters
import os
...
def compute_activations_across_models(models, train_loader, num_samples, dump_activations=False, dump_path=None):

    # hook that computes the mean activations across data samples
    def get_activation(activation, name):
        def hook(model, inputs, output):
            if type(inputs) == tuple:
                inputs = [inp.detach() for inp in inputs if type(inp) is torch.Tensor]
            if len(inputs) >= 1: # todo detect the useless hook.
                try:
                    if len(output) > 0:
    ...
    activations = {}
    excluded_list = ['.embeddings', '.prefix_tuning', 'prefix_tuning.pool', '.pooler'] #todo ignore embeddings and prefix_tuning',
                                                            #note that attention' is ok now, 'attention.self' is ok now,
    for idx, model in enumerate(models):
        ...
        for name, layer in model.named_modules():
            ...
            for elem in excluded_list:
                if not name or name.endswith(elem):
                    exclude = True
                    break
            if exclude:
                continue
...
def compute_activations_across_models_v1(args, models, train_loader, num_samples, mode='mean',
    ...
    # hook that computes the mean activations across data samples
    def get_activation(activation: dict, name):
        def hook(model, input, output):
            if name not in activation:
                activation[name] = []
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        for idx, model in enumerate(models):
            model(data)
        targets[target.item()] = targets.get(target.item(), 0)+1
        num_samples_processed += 1
    print(">>> statistics about the data used to generate activations:")
    print("skip_personal_idx:", args.skip_personal_idx)
