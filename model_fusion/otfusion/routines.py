...
def train(args, network, optimizer, train_loader, log_dict, epoch, model_id=-1):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        if batch_idx % args.rt_log_interval == 0:
            ...
            os.makedirs('{}/{}'.format(args.rt_result_dir, args.rt_exp_name), exist_ok=True)
            if args.rt_model_save:
                assert model_id != -1
                torch.save(network.state_dict(), '{}/{}/model_{}_{}.pth'.format(args.rt_result_dir, args.rt_exp_name, args.model_name, model_id))
                torch.save(optimizer.state_dict(), '{}/{}/optimizer_{}_{}.pth'.format(args.rt_result_dir, args.rt_exp_name, args.model_name, model_id))


def test(args, network, test_loader, return_loss=False, is_local=False):
    network.eval()
    test_loss = 0
    correct = 0
    if is_local:
