...
def compute_activations_across_models_zz(args, models, inputs, dump_activations=False, dump_path=None,
                                         activations=None):
...
def align_activations(args, activations):
    ...
    for name in activations[0].keys():
        ...
        if is_conv:
            ...
            feature_dim = -1
        T_var_in, _, _ = align_activation(args, activations[0][name]['input'], activations[1][name]['input'],
                                          feature_dim)
        T_var_out, _, _ = align_activation(args, activations[0][name]['output'], activations[1][name]['output'],
                                           feature_dim)
...
def fuse_model2(args, model, model1, activations_T, inplace=False, recompute_acts=False, acts_inputs=None,
                feature_dim=0):
    def rstrip(s, tail):
        if s.endswith(tail):
    ...
    with torch.no_grad():
        for (layer0_name, layer0_weight), (layer1_name, layer1_weight) in zip(model0.named_parameters(),
            ...
            if feature_dim == 0:
                layer0_aliged = torch.matmul(T_var_out.t(),
                                             layer0_aliged.reshape(layer0_weight_shape[0], -1).type(args.dtype)).float()
                # layer0_aliged = (T_var_out.t() @ layer0_aliged.reshape(layer0_weight_shape[0], -1).type(args.dtype)).float()
            elif feature_dim == -1:
                layer0_aliged = torch.matmul(layer0_aliged.reshape(-1, layer0_weight_shape[-1]).type(args.dtype),
                                             T_var_out).float()
...
def adaptive_fuse_model(args, model, model1, origin_activations, skip_out, skip_in=[], inplace=False,
                        recompute_acts=True, acts_inputs=None, act_feature_dim=-1):
    model0 = model if inplace else copy.deepcopy(model)
    aligned_params = {}
    avg_aligned_params = {}
    ...
    with torch.no_grad():
        for (layer0_name, layer0_weight), (layer1_name, layer1_weight) in zip(model0.named_parameters(),
            ...
            if name not in skip_in and layer0_weight.ndim != 1:
                T_var_in, _, _ = align_activation(args, origin_activations[0][name]['input'],
                                                  activations[0][name]['input'], feature_dim=act_feature_dim)
                # T_var_in = T_var_out
            if name not in skip_out:
                T_var_out, _, _ = align_activation(args, origin_activations[0][name]['output'],
                                                   activations[1][name]['output'], feature_dim=act_feature_dim)
            ...
            if recompute_acts and (T_var_in != None or T_var_out != None) and torch.trace(T_var_out) < 1 - 1e-4:
                print('[recompute]')
                activations = compute_activations_across_models_zz(args, (model0, model1),
                                                                   acts_inputs)  # , activations=activations
...
def adaptive_fuse_model2(args, model, model1, activation_manager, inplace=False, recompute_acts=True, skip_in=[],
                         skip_out=[], p0=0.5):
    model0 = model if inplace else copy.deepcopy(model)
    ...
    with torch.no_grad():
        for (layer0_name, layer0_weight), (layer1_name, layer1_weight) in zip(model0.named_parameters(),
            ...
            if recompute_acts and p_dis(layer0_aliged,
                                        layer0_weight) > 1e-6:  # (torch.trace(T_var_out)<(1-1e-4)*T_var_out.size(0) or torch.trace(T_var_in)<(1-1e-4)*T_var_in.size(0)):
    ...
    with torch.no_grad():
        for (layer0_name, layer0_weight) in model0.named_parameters():
            ...
            if name in skip_in:
                ...
                am.activations[0][name]['input']
                T_var_in, _, _ = align_activation(args, am.origin_activations[0][name]['input'],
                                                  am.activations[0][name]['input'], feature_dim=act_feature_dim)
            ...
            am.activations = compute_activations_across_models_zz(args, [model0],
                                                                  acts_inputs)  # , activations=activations
...
def tune_activations_T_for_transformers(activations_T_x):
    ...
    for layer_idx in range(6):
        ...
        activations_T[f'encoder.layers.{layer_idx}.self_attn.q_proj']['input'] = \
        activations_T[f'encoder.{prev_last_module}']['output']
        activations_T[f'encoder.layers.{layer_idx}.self_attn.k_proj']['input'] = \
        activations_T[f'encoder.{prev_last_module}']['output']
        activations_T[f'encoder.layers.{layer_idx}.self_attn.v_proj']['input'] = \
        activations_T[f'encoder.{prev_last_module}']['output']

        activations_T[f'decoder.layers.{layer_idx}.self_attn.q_proj']['input'] = \
        activations_T[f'decoder.{prev_last_module}']['output']
        activations_T[f'decoder.layers.{layer_idx}.self_attn.k_proj']['input'] = \
        activations_T[f'decoder.{prev_last_module}']['output']
        activations_T[f'decoder.layers.{layer_idx}.self_attn.v_proj']['input'] = \
        activations_T[f'decoder.{prev_last_module}']['output']

        activations_T[f'encoder.layers.{layer_idx}.self_attn.q_proj']['output'] = \
        activations_T[f'encoder.layers.{layer_idx}.self_attn.k_proj']['output']
        activations_T[f'decoder.layers.{layer_idx}.self_attn.q_proj']['output'] = \
        activations_T[f'decoder.layers.{layer_idx}.self_attn.k_proj']['output']
        activations_T[f'decoder.layers.{layer_idx}.encoder_attn.q_proj']['output'] = \
        activations_T[f'decoder.layers.{layer_idx}.encoder_attn.k_proj']['output']

        activations_T[f'encoder.layers.{layer_idx}.self_attn.v_proj']['output'] = \
        activations_T[f'encoder.layers.{layer_idx}.self_attn.k_proj']['output']  # 感觉不需要
        activations_T[f'decoder.layers.{layer_idx}.self_attn.v_proj']['output'] = \
        activations_T[f'decoder.layers.{layer_idx}.self_attn.k_proj']['output']
        activations_T[f'decoder.layers.{layer_idx}.encoder_attn.v_proj']['output'] = \
        activations_T[f'decoder.layers.{layer_idx}.encoder_attn.k_proj']['output']

        activations_T[f'encoder.layers.{layer_idx}.self_attn.out_proj']['input'] = \
        activations_T[f'encoder.layers.{layer_idx}.self_attn.v_proj']['output']
        activations_T[f'decoder.layers.{layer_idx}.self_attn.out_proj']['input'] = \
        activations_T[f'decoder.layers.{layer_idx}.self_attn.v_proj']['output']
        activations_T[f'decoder.layers.{layer_idx}.encoder_attn.out_proj']['input'] = \
        activations_T[f'decoder.layers.{layer_idx}.encoder_attn.v_proj']['output']

        activations_T[f'encoder.layers.{layer_idx}.self_attn_layer_norm']['input'] = \
        activations_T[f'encoder.layers.{layer_idx}.self_attn.out_proj']['output']
        activations_T[f'decoder.layers.{layer_idx}.self_attn_layer_norm']['input'] = \
        activations_T[f'decoder.layers.{layer_idx}.self_attn.out_proj']['output']
        activations_T[f'decoder.layers.{layer_idx}.encoder_attn_layer_norm']['input'] - \
        activations_T[f'decoder.layers.{layer_idx}.encoder_attn.out_proj']['output']

        activations_T[f'encoder.layers.{layer_idx}.fc1']['input'] = \
        activations_T[f'encoder.layers.{layer_idx}.self_attn_layer_norm']['output']  # 本来就差不多
        activations_T[f'decoder.layers.{layer_idx}.fc1']['input'] = \
        activations_T[f'decoder.layers.{layer_idx}.encoder_attn_layer_norm']['output']

        activations_T[f'encoder.layers.{layer_idx}.fc2']['input'] = activations_T[f'encoder.layers.{layer_idx}.fc1'][
            'output']
        activations_T[f'decoder.layers.{layer_idx}.fc2']['input'] = activations_T[f'decoder.layers.{layer_idx}.fc1'][
            'output']

        activations_T[f'encoder.layers.{layer_idx}.final_layer_norm']['input'] = \
        activations_T[f'decoder.layers.{layer_idx}.fc2']['output']
        activations_T[f'decoder.layers.{layer_idx}.final_layer_norm']['input'] = \
        activations_T[f'decoder.layers.{layer_idx}.fc2']['output']
...
def check_activations(activations):
    ...
    for key in activations[0].keys():
        try:
            print(
                f"{key}: input shape, {activations[0][key]['input'][0].shape}, output shape{activations[0][key]['output'][0].shape}")
...
def check_model_parameters(model, activation_manager):
    for name, p in model.named_parameters():
        ...
        print(f"\n>>> {m_name}")
        try:
            print(activation_manager.activations[0][m_name]['input'].shape,
                  activation_manager.activations[0][m_name]['output'].shape)
        except Exception as e:
            print(e)
...
def activation_diff(activationManager):
    for name in activationManager.origin_activations[0].keys():
        ...
        if 'output' in activationManager.origin_activations[0][name].keys() and type(
                activationManager.origin_activations[0][name]['output']) == torch.Tensor:
...
if __name__ == '__main__':
    ...
    fused_model, aligned_model, actural_acts_T, new_activation = adaptive_fuse_model(args, model0, model1, activations,
                                                                                     acts_inputs=acts_inputs,
                                                                                     recompute_acts=True,
                                                                                     skip_out=['fc4'])
