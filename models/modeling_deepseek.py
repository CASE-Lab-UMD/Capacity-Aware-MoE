...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        if seq_len == 1 or self.expert_capacity is None: 
            expert_capacity = None        
        elif self.expert_capacity is not None: 
            expert_capacity = math.ceil(self.expert_capacity * self.top_k * (total_tokens / self.n_routed_experts))
        else: 
    ...
    def overflow(self, utilized_mask, expert_capacity): 
        ...
        # print(f"{self.layer_idx} expert_capacity: {expert_capacity}") ### these two rows are important for checking capacity. 
        # print(f"{self.layer_idx} flow: {token_priority[-1, :].tolist()}")
    ...
    def adjust_tokens(self, expert_capacity, scores): 
        ...
        # print(f"nan numbers: {nan_mask.float().mean()}")
        # topk_weight, topk_idx = torch.topk(scores, k=self.top_k, dim=-1, sorted=False)
        # return topk_weight, topk_idx
        # nan_mask = torch.isnan(scores)
        # print(f"nan numbers line 539: {nan_mask.float().mean()}")
        # expert_capacity = None
        ...
        nan_mask = torch.isnan(scores)
        # print(f"nan numbers line 549: {nan_mask.float().mean()}")
        ...
        # TODO Drop first tokens, 
            ...
            # nan_mask = torch.isnan(scores)
            # print(f"nan numbers before: {nan_mask.float().mean()}")
            for i in range(utilized_mask.size()[-1]):
                if overflow[i] > 0: 
                    k = overflow[i].item()
                    sub_mask = utilized_mask[:, i]
                    # sliced = sub_mask[sub_mask]
                    importance_scores = torch.randn_like(scores[sub_mask, i])
                    threshold, _ = torch.kthvalue(importance_scores, k)
                    # print(f"importance_scores: {importance_scores}")
                    # print(f"threshold: {threshold}")
                    # print(f"importance_scores > threshold: {(importance_scores > threshold)}")
                    # print(f"scores[:, {i}]: {scores[:, i]}")
                    utilized_mask[sub_mask, i] = importance_scores > threshold
                    # utilized_mask = mask.clone()
                    # print(f"utilized_mask[sub_mask, i]: {utilized_mask[sub_mask, i]}")
                    # print(f"scores[:, {i}]: {scores[:, i]}")
                    
            nan_mask = torch.isnan(scores)
            # print(f"nan numbers line 592: {nan_mask.float().mean()}")
            utilized_mask = utilized_mask[mask].view(topk_idx.size())
            # topk_idx[~utilized_mask] = self.n_routed_experts 
            topk_weight = topk_weight * utilized_mask
            # nan_mask = torch.isnan(scores)
            # print(f"nan numbers line 601: {nan_mask.float().mean()}")
            # nan_mask = torch.isnan(scores)
            # print(f"nan numbers after: {nan_mask.float().mean()}")
            # print()
            ...
            for r in range(rounds):
                ...
                for i in range(utilized_mask.size()[-1]):
                    if overflow[i] > 0: 
                        ...
                        # print(f"importance_scores: {importance_scores.tolist()}")
                        # print(f"threshold: {threshold}")
                ...
                # print(f"new_overflow: {new_overflow}")
                utilized_mask = utilized_mask[mask].view(topk_idx.size())
                # topk_idx[~utilized_mask] = self.n_routed_experts
                topk_weight = topk_weight * utilized_mask
...
class DeepseekV2MoE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        ### 🔍🔍🔍
        topk_idx, topk_weight, aux_loss, expert_idxes = self.gate(hidden_states)
        ###
        ...
        if self.training:
            ...
            y = self.moe_infer(hidden_states, topk_idx, topk_weight, expert_idxes).view(*orig_shape)
