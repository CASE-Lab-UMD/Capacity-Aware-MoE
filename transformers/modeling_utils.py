...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def _load_state_dict_into_model(cls, model, state_dict, pretrained_model_name_or_path, _fast_init=True):
        ...
        torch.manual_seed(model.config.seed) # todo maintain the consistence between single model and moe.
        # todo load the weight in the experts
        if hasattr(model.config, 'use_moe') and model.config.use_moe:
            load_MoE(model, state_dict, loaded_keys, missing_keys, unexpected_keys, model.config.model_type)
...
class Split_Conv1D(nn.Module):
    ...
    def forward(self, x, weight, mask, phi):
        size_out = x.size()[:-1] + (self.nf,)
        x = torch.addmm(self.bias, x.view(-1, x.size(-1)), self.weight)
        # x = torch.addmm(self.bias, x.view(-1, x.size(-1)), 2 * self.weight * mask * phi + 2 * weight * (1 - mask) * (1 - phi))
        x = x.view(*size_out)
        return x
...
def apply_chunking_to_forward(
    forward_fn: Callable[..., torch.Tensor], chunk_size: int, chunk_dim: int, *input_tensors, synflow=None
) -> torch.Tensor:
    """
...
def load_MoE(model, state_dict, loaded_keys, missing_keys, unexpected_keys, model_type):
    loaded_keys.sort()
    missing_keys.sort()
    unexpected_keys.sort()
    if model_type == 'bert' or model_type == 'roberta' or model_type == 'electra':
        moe_note = 'output.dense'
        for key in loaded_keys:  # todo load experts
            if moe_note in key and 'attention' not in key:
                values = state_dict.pop(key, None)
                if key in unexpected_keys:
                    unexpected_keys.remove(key)
                for layer in range(model.config.n_experts):
                    moe_key = key.replace('.dense.', '.dense.experts.' + str(layer) + '.dense.')  # used in bert
                    state_dict[moe_key] = values
                    # todo remove moe_key in single model
                    if moe_key in missing_keys:
                        missing_keys.remove(moe_key)
        ...
        for key in loaded_keys:  # todo load experts
            if moe_note in key and 'attention' not in key:
                ...
                if key in unexpected_keys:
                    unexpected_keys.remove(key)
                for layer in range(model.config.n_experts):
                    moe_key = key.replace('.ffn_output.', '.ffn_output.experts.' + str(layer) + '.dense.')  # used in albert
                    state_dict[moe_key] = values
                    # todo remove moe_key in single model
                    if moe_key in missing_keys:
                        missing_keys.remove(moe_key)
        ...
        for key in loaded_keys:  # todo load experts
            if moe_note in key and 'attention' not in key:
                ...
                if key in unexpected_keys:
                    unexpected_keys.remove(key)
                for layer in range(model.config.n_experts):
                    moe_key = key.replace('.fc2.', '.fc2.experts.' + str(layer) + '.dense.')  # used in albert
                    state_dict[moe_key] = values
                    # todo remove moe_key in single model
                    if moe_key in missing_keys:
                        missing_keys.remove(moe_key)
    # todo remove moe elements in missing_keys
    moe_notes = ['dense.mean', 'dense.std', 'dense.w_gate', 'dense.w_noise']
    moe_keys = []
    for key in missing_keys:
        for note in moe_notes:
            if note in key:
                moe_keys.append(key)
    for key in moe_keys:
        missing_keys.remove(key)

    # for key in missing_keys:  # todo load noise
    #     if 'classifier' not in key:
    #         state_dict[key] = model_dict[key]

# def load_MoE_gpt2(model, state_dict, loaded_keys, missing_keys, unexpected_keys, prefix):
...
#         if '.mlp.' in key:
#             for layer in range(model.config.n_experts):
#                 moe_key = key.replace('.mlp.', '.mlp.experts.' + str(layer) + '.')
#                 state_dict[moe_key] = state_dict[key]
#                 missing_keys.remove(moe_key)
#             # todo avoid the weight to be same with the experts
#             if 'bias' not in key:
#                 moe_key = key.replace('.weight', '_weight')
#                 state_dict[moe_key] = state_dict[key]
#                 missing_keys.remove(moe_key)
#                 unexpected_keys.remove(key)
#                 state_dict.pop(key)
...
#     for key in missing_keys:  # todo load noise
#         if 'classifier' not in key:
#             state_dict[key] = model_dict[prefix + '.' + key]
