...
class AlbertLayer(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, 'use_moe') and config.use_moe:
            self.ffn_output = ALBertMoE(config.intermediate_size, config)
        else:
            self.ffn_output = nn.Linear(config.intermediate_size, config.hidden_size)
    ...
    def forward(
        ...
        if hasattr(self.config, 'use_moe') and self.config.use_moe:
            ffn_output, moe_loss = apply_chunking_to_forward(
                self.ff_chunk,
        ...
        if hasattr(self.config, 'use_moe') and self.config.use_moe:
            return (hidden_states,) + attention_output[1:], moe_loss  # add attentions if we output them
        else:
    ...
    def ff_chunk(self, attention_output):
        ...
        if hasattr(self.config, 'use_moe') and self.config.use_moe:
            ffn_output, moe_loss = self.ffn_output(ffn_output)
            return ffn_output, moe_loss
        else:
...
class AlbertLayerGroup(nn.Module):
    ...
    def forward(
        ...
        layer_hidden_states = ()
        layer_attentions = ()
        moe_loss = 0 if (hasattr(self.config, 'use_moe') and self.config.use_moe) else None

        for layer_index, albert_layer in enumerate(self.albert_layers):
            if hasattr(self.config, 'use_moe') and self.config.use_moe:
                layer_output, loss = albert_layer(hidden_states, attention_mask, head_mask[layer_index], output_attentions)
                moe_loss += loss
            else:
        ...
        if hasattr(self.config, 'use_moe') and self.config.use_moe:
            return outputs, moe_loss  # last-layer hidden state, (layer hidden states), (layer attentions)
        else:
            return outputs
...
class AlbertTransformer(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            if hasattr(self.config, 'use_moe') and self.config.use_moe:
                layer_group_output, loss = self.albert_layer_groups[group_idx](
                    hidden_states,
                    attention_mask,
        ...
        return BaseModelOutput(
            last_hidden_state=hidden_states, hidden_states=all_hidden_states, attentions=all_attentions, moe_loss=moe_loss
        )
...
class AlbertModel(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        moe_loss = encoder_outputs['moe_loss'] if (hasattr(self.config, 'use_moe') and self.config.use_moe) else None
...
class AlbertForMaskedLM(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        if not self.config.use_moe:
            ...
            sequence_outputs, moe_loss = outputs[0], outputs[1]
        ...
        return MaskedLMOutput(
            loss=masked_lm_loss + moe_loss if moe_loss else masked_lm_loss,
            moe_loss=moe_loss,
...
class AlbertForSequenceClassification(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        moe_loss = outputs['moe_loss'] if (hasattr(self.config, 'use_moe') and self.config.use_moe) else None
        ...
        return SequenceClassifierOutput(
            loss=loss + moe_loss if moe_loss else loss,
            moe_loss=moe_loss,
            logits=logits,
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
...
class ALBertMoE(nn.Module):
    ...
    def __init__(self, intermediate_size, config, noisy_gating=True, device='cpu'):
        ...
        self.w_noise = nn.Parameter(torch.zeros(intermediate_size, config.n_experts), requires_grad=True).to(self.device)
...
class SingleExpert(nn.Module):
    ...
    def forward(self, hidden_states):
        hidden_states = F.linear(hidden_states, self.dense.weight, self.dense.bias)
