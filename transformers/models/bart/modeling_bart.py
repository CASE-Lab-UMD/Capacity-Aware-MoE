...
class BartEncoderLayer(nn.Module):
    def __init__(self, config: BartConfig, layer_id=None):
        ...
        if config.use_moe:
            self.fc2 = BartMoE(config.encoder_ffn_dim, config)
        else:
    ...
    def forward(
        ...
        if self.config.use_moe:
            hidden_states, moe_loss = self.fc2(hidden_states)
        else:
        ...
        if self.config.use_moe:
            return outputs, p_prime, moe_loss
        else:
    ...
    def forward(
        ...
        Args:
            ...
            return_dict (:obj:`bool`, `optional`):
                Whether or not to return a :class:`~transformers.file_utils.ModelOutput` instead of a plain tuple.
        """
        moe_loss = 0 if self.config.use_moe else None
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                if getattr(self.config, "gradient_checkpointing", False) and self.training:
                    ...
                    if self.config.use_moe:
                        layer_outputs, p_prime, loss = encoder_layer(
                            hidden_states,
                            attention_mask,
        ...
        return BaseModelOutput(
            last_hidden_state=hidden_states, hidden_states=encoder_states, attentions=all_attentions, moe_loss=moe_loss)
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                if self.config.use_moe:
                    layer_outputs, loss = decoder_layer(
                        hidden_states,
                        attention_mask=attention_mask,
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            en_moe_loss = encoder_outputs['moe_loss'] if self.config.use_moe else None
        ...
        de_moe_loss = decoder_outputs['moe_loss'] if self.config.use_moe else None
        moe_loss = en_moe_loss + de_moe_loss if self.config.use_moe else None
        if not return_dict:
...
class BartMoE(nn.Module):
    def __init__(self, intermediate_size, config, noisy_gating=True, device='cpu'):
        ...
        self.weight = nn.Parameter(torch.ones(config.hidden_size, intermediate_size), requires_grad=True)
        ...
        self.register_buffer('mask', torch.ones(config.hidden_size, intermediate_size))
    ...
    def forward(self, x, loss_coef=1e-2):
        ...
        expert_outputs = [self.experts[i](expert_inputs[i], self.weight) for i in range(self.num_experts)]
...
class SingleExpert(nn.Module):
    ...
    def forward(self, hidden_states):
        return F.linear(hidden_states, self.dense.weight, self.dense.bias)
