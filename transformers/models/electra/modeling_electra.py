...
class ElectraModel(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        moe_loss = hidden_states['moe_loss'] if (hasattr(self.config, 'use_moe') and self.config.use_moe) else None
        if hasattr(self.config, 'use_moe') and self.config.use_moe:
            return hidden_states, moe_loss
        else:
            return hidden_states
...
class ElectraForSequenceClassification(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        moe_loss = None
        if hasattr(self.config, 'use_moe') and self.config.use_moe:
            discriminator_hidden_states, moe_loss = self.electra(
                input_ids,
                attention_mask,
                token_type_ids,
                position_ids,
...
class ElectraForMaskedLM(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        if not self.config.use_moe:
            ...
            generator_hidden_states, moe_loss = generator_hidden_states[0], generator_hidden_states[1]
            generator_sequence_output = generator_hidden_states[0]
