...
class GPT2Block(nn.Module):
    def __init__(self, config):
        ...
        if config.use_moe:
            self.mlp = GPT2MOE(inner_dim, config)
        else:
    ...
    def forward(
        ...
        # nan = (hidden_states != hidden_states).float().sum()
        # inf = torch.isinf(hidden_states).float().sum()
        hidden_states = self.ln_1(hidden_states)
        # nan_after_ln = (hidden_states != hidden_states).float().sum()
        # inf_after_ln = torch.isinf(hidden_states).float().sum()
        ...
        if override and override['type'] == 'ffn_input_after_ln':
        ...
        if self.config.use_moe:
            feed_forward_hidden_states, loss = self.mlp(hidden_states)
        else:
        ...
        if self.config.use_moe:
            return outputs, loss # hidden_states, present, (attentions, cross_attentions)
        else:
            return outputs
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                if self.config.use_moe:
                    outputs, loss = torch.utils.checkpoint.checkpoint(
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        moe_loss = transformer_outputs['moe_loss'] if self.config.use_moe else None
    ...
    def reset_buffers(self):
        for name, module in self.base_model.named_modules():
            if isinstance(module, (GPT2MOE)):
                print("tune " + name + '.c_fc_mask')
                mask = module.c_fc_mask
                module.register_buffer('c_fc_mask', mask)
                print("tune " + name + '.c_proj_mask')
                mask = module.c_proj_mask
                module.register_buffer('c_proj_mask', mask)
