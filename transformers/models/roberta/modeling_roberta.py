...
class RobertaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        mixed_query_layer = self.query(hidden_states) # inf emerge
...
class RobertaOutput(nn.Module):
    def __init__(self, config):
        ...
        if hasattr(config, 'use_moe') and config.use_moe:
            self.dense = RobertaMoE(config.intermediate_size, config)
        else:
    ...
    def forward(self, hidden_states, input_tensor):
        if hasattr(self.config, 'use_moe') and self.config.use_moe:
            hidden_states, moe_loss = self.dense(hidden_states)
        else:
            hidden_states = self.dense(hidden_states)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
...
class RobertaLayer(nn.Module):
    ...
    def forward(
        ...
        if hasattr(self.config, 'use_moe') and self.config.use_moe:
            layer_output, moe_loss = apply_chunking_to_forward(
    ...
    def feed_forward_chunk(self, attention_output):
        ...
        if hasattr(self.config, 'use_moe') and self.config.use_moe:
            layer_output, moe_loss = self.output(intermediate_output, attention_output)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                if hasattr(self.config, 'use_moe') and self.config.use_moe:
                    layer_outputs, loss = torch.utils.checkpoint.checkpoint(
                ...
                if hasattr(self.config, 'use_moe') and self.config.use_moe:
                    layer_outputs, loss = layer_module(
                        hidden_states,
                        attention_mask,
                        layer_head_mask,
                        encoder_hidden_states,
