...
from transformers.pruning.prune import prune_loop
from transformers.pruning.generator import *
class Trainer_Split:
    """
    ...
    def __init__(
        ...
        prune_args: PruneArguments = None,
        tune_args: TuneArguments = None,
        ...
        self.device_ids = [int(did) for did in self.args.device_ids.split(' ')]
        self.device = torch.device("cuda:{}".format(self.device_ids[0]))
    ...
    def get_prune_dataloader(self) -> DataLoader:
        """
        Returns the training :class:`~torch.utils.data.DataLoader`.
        ...
        prune_size = int(length * 32)
        rest_size = len(train_dataset) - prune_size
        prune_dataset, _ = torch.utils.data.random_split(train_dataset, [prune_size, rest_size])
        batch_size = self.args.train_batch_size if self.prune_args.pruner != 'synflow' else 1
    ...
    def _wrap_model(self, model, training=True):
        ...
        # self.args.local_rank = len(self.device_ids)
    ...
    def train(
        ...
        **kwargs,
        ...
        prune_dadaloader = self.get_prune_dataloader()
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            if is_torch_tpu_available():
                parallel_loader = pl.ParallelLoader(train_dataloader, [self.device]).per_device_loader(self.device)
            ...
            if epoch == 0 and model.module.config.use_moe and self.prune_args.sparsity < 1:
                # todo pruning the whole adapter in moe
                pruner = self.pruner(self.prune_args.pruner)(masked_parameters(model))
                pruner.optimizer = self.optimizer
                device = model.module.device
                prune_loop(model, pruner, prune_dadaloader, device, self.prune_args.sparsity,
                           self.prune_args.compression_schedule, self.prune_args.mask_scope,
                           self.prune_args.prune_epochs,
                           self.prune_args.reinitialize, self.prune_args.prune_train_mode, self.prune_args.shuffle,
                           self.prune_args.invert, self.compute_loss)
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
        ...
        if os.path.isfile(os.path.join(checkpoint, "optimizer.pt")) and os.path.isfile(
            ...
            if is_torch_tpu_available():
                ...
                map_location = "cpu" if is_sagemaker_mp_enabled() else self.device
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if is_sagemaker_mp_enabled():
            ...
            return loss_mb.reduce_mean().detach().to(self.device)
    ...
    def evaluate(
        ...
        self.resume_checkpoints(resume_from_checkpoint)
        output = eval_loop(
            eval_dataloader,
        ...
        # features_avg = np.array([item.cpu().detach().numpy() for item in self.feature_avg])
        np.save(os.path.join(resume_from_checkpoint, 'feature_avg'), self.feature_avg)
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    ...
                    hidden_states = [hidden_states.detach().cpu() for hidden_states in outputs['hidden_states']]
                    self.feature_avg = hidden_states if not self.feature_avg \
                        else [np.concatenate((self.feature_avg[i], hidden_states[i]), axis=0) for i in range(len(hidden_states))]
    ...
    def resume_checkpoints(self, resume_from_checkpoint):
        if resume_from_checkpoint is not None:
            if not os.path.isfile(os.path.join(resume_from_checkpoint, WEIGHTS_NAME)):
                raise ValueError(f"Can't find a valid checkpoint at {resume_from_checkpoint}")
